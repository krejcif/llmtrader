"""Decision Agent - XRP prompt version (AI decides independently)"""
import sys
import os
...
def make_decision_xrp(state: TradingState) -> TradingState:
    """
    Make trading decision using XRP prompt (AI decides independently)
    
    Args:
        state: Current trading state with market data and analysis
        
    Returns:
        Updated state with trading recommendation (xrp strategy)
    """
    ...
    if FORCE_LONG_TEST:
        print(f"\n⚠️  [XRP STRATEGY - TEST MODE] FORCING LONG for paper trading test!")
        ...
        if not market_data or not analysis:
            return {"recommendation_xrp": None}
        ...
        return {"recommendation_xrp": recommendation}
    
    # Normal flow (when FORCE_LONG_TEST = False)
    print(f"\n🤖 [XRP STRATEGY] Making decision with DeepSeek AI...")
