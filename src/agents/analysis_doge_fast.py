"""Analysis for Doge Fast Strategy - Custom EMA 7/25"""
import sys
import os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from models.state import TradingState
from utils.indicators import calculate_all_indicators_custom, analyze_orderbook, detect_trend_reversal
...
def analyze_market_doge_fast(state: TradingState, strategy_name: str,
                                 tf_higher: str, tf_lower: str) -> TradingState:
    """
    Market analysis for Doge Fast strategy with custom EMA 7/25
    
    Args:
        state: Current trading state
        strategy_name: Name of strategy (for state keys)
        tf_higher: Higher timeframe (e.g., '1h')
        tf_lower: Lower timeframe (e.g., '15m')
    ...
    print(f"\n🔬 [{strategy_name.upper()}] Analyzing market data ({tf_higher}/{tf_lower}) - EMA 7/25...")
    ...
    try:
        ...
        # Calculate technical indicators with EMA 7/25
        print(f"   Analyzing {tf_higher} (trend) with EMA 7/25...")
        indicators_higher = calculate_all_indicators_custom(candles_higher, ema_periods=[7, 25])
        
        print(f"   Analyzing {tf_lower} (entry) with EMA 7/25...")
        indicators_lower = calculate_all_indicators_custom(candles_lower, ema_periods=[7, 25])
        ...
        # MOMENTUM ANALYSIS (for 5m strategies)
        momentum_data = None
        if tf_lower == "5m":
        ...
        print(f"✅ [{strategy_name.upper()}] Analysis complete (EMA 7/25):")
