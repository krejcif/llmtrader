"""Decision Agent - Doge Fast (EMA 7/25 instead of 20/50)"""
import sys
import os
...
def make_decision_doge_fast(state: TradingState) -> TradingState:
    """
    ...
    Returns:
        Updated state with trading recommendation (doge_fast strategy)
    """
    
    print(f"\n🤖 [DOGE_FAST STRATEGY] Making decision with DeepSeek AI (EMA 7/25)...")
...
{{
  ...
  "key_factors": ["factor1", "factor2", "factor3"]
        ...
        if recommendation['action'] in ['LONG', 'SHORT']:
            ...
            recommendation['strategy'] = 'doge_fast'  # Tag strategy
            recommendation['symbol'] = state['symbol']  # Add symbol
            
            print(f"\n✅ [DOGE_FAST] Decision made:")
            ...
            recommendation['strategy'] = 'doge_fast'
            recommendation['symbol'] = state['symbol']  # Add symbol
            print(f"\n✅ [DOGE_FAST] Decision made:")
            print(f"   Symbol: {state['symbol']}")
            print(f"   Action: {recommendation['action']}")
        ...
        return {"recommendation_doge_fast": recommendation}
        
    except json.JSONDecodeError as e:
        error_msg = f"Error parsing AI response: {str(e)}"
        print(f"❌ {error_msg}")
        return {"recommendation_doge_fast": None}
    except Exception as e:
        error_msg = f"Error making decision (doge_fast): {str(e)}"
        print(f"❌ {error_msg}")
        return {"recommendation_doge_fast": None}
