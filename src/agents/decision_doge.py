"""Decision Agent - Doge prompt version (AI decides independently)"""
import sys
import os
...
def make_decision_doge(state: TradingState) -> TradingState:
    """
    Make trading decision using DOGE prompt (AI decides independently)
    
    Args:
        state: Current trading state with market data and analysis
        
    Returns:
        Updated state with trading recommendation (doge strategy)
    """
    ...
    if FORCE_LONG_TEST:
        ...
        if not market_data or not analysis:
            return {"recommendation_doge": None}
        ...
        print(f"✅ [DOGE - TEST] Forced LONG decision:")
        ...
        return {"recommendation_doge": recommendation}
    
    # Normal flow (when FORCE_LONG_TEST = False)
    print(f"\n🤖 [DOGE STRATEGY] Making decision with DeepSeek AI...")
    ...
    try:
        ...
        # DOGE PROMPT - Just data, no rules
...
{{
  ...
  "key_factors": ["factor1", "factor2", "factor3"]
        ...
        if recommendation['action'] in ['LONG', 'SHORT']:
            ...
            recommendation['strategy'] = 'doge'  # Tag strategy
