"""Decision Agent - XRP Fast (EMA 7/25 instead of 20/50)"""
import sys
import os
...
def make_decision_xrp_fast(state: TradingState) -> TradingState:
    """
    ...
    Returns:
        Updated state with trading recommendation (xrp_fast strategy)
    """
    
    print(f"\n🤖 [XRP_FAST STRATEGY] Making decision with DeepSeek AI (EMA 7/25)...")
...
{{
  ...
  "key_factors": ["factor1", "factor2", "factor3"]
        ...
        if recommendation['action'] in ['LONG', 'SHORT']:
            ...
            recommendation['strategy'] = 'xrp_fast'  # Tag strategy
            recommendation['symbol'] = state['symbol']  # Add symbol
            
            print(f"\n✅ [XRP_FAST] Decision made:")
            ...
            recommendation['strategy'] = 'xrp_fast'
            recommendation['symbol'] = state['symbol']  # Add symbol
            print(f"\n✅ [XRP_FAST] Decision made:")
            print(f"   Symbol: {state['symbol']}")
            print(f"   Action: {recommendation['action']}")
        ...
        return {"recommendation_xrp_fast": recommendation}
        
    except json.JSONDecodeError as e:
        error_msg = f"Error parsing AI response: {str(e)}"
        print(f"❌ {error_msg}")
        return {"recommendation_xrp_fast": None}
    except Exception as e:
        error_msg = f"Error making decision (xrp_fast): {str(e)}"
        print(f"❌ {error_msg}")
        return {"recommendation_xrp_fast": None}
