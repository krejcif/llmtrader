"""Decision Agent - ETH strategy (Minimal for ETHUSDT)"""
import sys
import os
...
def make_decision_eth(state: TradingState) -> TradingState:
    """
    Make trading decision using MINIMAL prompt for ETH (AI decides independently)
    
    Args:
        state: Current trading state with market data and analysis
        
    Returns:
        Updated state with trading recommendation (eth strategy)
    """
    ...
    if FORCE_LONG_TEST:
        print(f"\n⚠️  [ETH STRATEGY - TEST MODE] FORCING LONG for paper trading test!")
        ...
        if not market_data or not analysis:
            return {"recommendation_eth": None}
        ...
        print(f"✅ [ETH - TEST] Forced LONG decision:")
        ...
        return {"recommendation_eth": recommendation}
    
    # Normal flow (when FORCE_LONG_TEST = False)
    print(f"\n🤖 [ETH STRATEGY] Making decision with DeepSeek AI...")
...
{{
  ...
  "key_factors": ["factor1", "factor2", "factor3"]
        ...
        if recommendation['action'] in ['LONG', 'SHORT']:
            ...
            recommendation['strategy'] = 'eth'  # Tag strategy
        ...
        error_msg = f"Error making decision (eth): {str(e)}"
        print(f"❌ {error_msg}")
        return {"recommendation_eth": None}
